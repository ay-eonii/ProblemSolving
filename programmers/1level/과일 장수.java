...
class Solution {
    public int solution(int k, int m, int[] score) {
        ...
        Arrays.sort(score);
        int benefit = 0;
        for (int i = remain; i < score.length; i += m) {
            benefit += score[i] * m;
        }
        return benefit;
    }
...
/*
 * strategy
 * 1. 남는 사과 개수: score.length % m
 * 1-1. 팔지 못하는(나머지) 사과가 있다면 최저 사과를 남기자: score.length - (score.length % m)
 * 2. 정렬(오름차순) -> 최저 점수
 * 3. 최대이익을 위해서는? 최저끼리 묶기
 * 4. return sum(묶은 사과에서 최저점수 * m)
 */