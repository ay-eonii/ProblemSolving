...
class Solution {
    public int[] solution(int N, int[] stages) {
        ...
        for (int i = 1; i <= N; i++) {
            ...
            float failureRate = (player == 0) ? 0 : (float) failPlayer / player;
            failureRates.put(i, failureRate);
        }

        List<Integer> sortedStages = new ArrayList<>(failureRates.keySet());
        sortedStages.sort((a, b) -> failureRates.get(b).compareTo(failureRates.get(a)));

        int[] result = new int[sortedStages.size()];
...
/*
 * strategy
 * 1. 각 스테이지 실패율 구하기: for (1 ~ N) -> key:Lv, value:실패율
 * 1-1. 스테이지에 도달한 플레이어 수 구하기: 해당 스테이지보다 크거나 같은 경우
 * 1-2. 스테이지에 도달했으나 클리어 하지 못한 플레이어 수 구하기: 해당 스테이지와 같은 경우
 * 2. 실패율 내림차순
 * 
 * 3. 실패율 같은 경우 -> 레벨 오름차순
 */
